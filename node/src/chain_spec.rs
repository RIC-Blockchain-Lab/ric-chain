...
pub fn ric_staging_testnet_config() -> ChainSpec {
	let boot_nodes = vec![];
	...
	ChainSpec::from_genesis(
		...
		"ric_staging",
		ChainType::Live,
		ric_staging_testnet_genesis,
		boot_nodes,
		Some(
			TelemetryEndpoints::new(vec![(RIC_STAGING_TELEMETRY_URL.to_string(), 0)])
...
fn ric_staging_testnet_genesis() -> GenesisConfig {
